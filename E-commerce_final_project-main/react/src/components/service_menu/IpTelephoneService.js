...
const IpTelephoneService = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:1369/api/ip-telephone-service?populate=deep,10');
    ...
    const fetchHomeData = async () => {
      try {
        const responseHome = await fetch('http://localhost:1369/api/homes?populate=deep,10');
